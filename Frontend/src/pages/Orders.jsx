...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border shadow-sm ${statusStyles[status] || "bg-slate-50 text-slate-700 border-slate-100"}`}>
      {statusIcons[status]}
      {status}
    </span>
  );
  ...
  return (
    <div 
      className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
        isExpanded ? 'border-indigo-200 shadow-xl shadow-indigo-500/5' : 'border-slate-100 hover:border-indigo-100 hover:shadow-lg'
      } mb-6`}
    >
      <div 
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center justify-between md:justify-end gap-10 border-t md:border-t-0 pt-4 md:pt-0">
            ...
            <div className={`p-3 rounded-xl bg-slate-50 text-slate-400 transition-all duration-300 ${isExpanded ? 'rotate-180 bg-indigo-50 text-indigo-600' : ''}`}>
              <ChevronDown size={20} />
            </div>
          </div>
        </div>
                ...
                {order.items?.map((item, idx) => (
                  <div key={idx} className="flex gap-5 group">
                    <div className="w-20 h-24 bg-slate-50 rounded-xl overflow-hidden border border-slate-100 shrink-0">
                      <img 
                        src={item.snapshot?.image || "https://via.placeholder.com/80x96"} 
                    ...
                    <div className="flex-1 py-1">
                      <div className="flex justify-between items-start gap-4 mb-2">
                        ...
                        <span className="text-lg font-bold text-slate-900">
                          ₹{(item.snapshot?.paidPrice * item.quantity).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold text-slate-500">
                        {item.snapshot?.size && (
                          <span className="flex items-center gap-1.5 px-2 py-1 bg-slate-50 rounded-md">
                            Size: <span className="text-slate-900 uppercase">{item.snapshot.size}</span>
                          </span>
                        )}
                        {item.snapshot?.color && (
                          <span className="flex items-center gap-1.5 px-2 py-1 bg-slate-50 rounded-md">
                            Color: <span className="text-slate-900">{item.snapshot.color}</span>
                          </span>
                        )}
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-3">
                        Unit Price: ₹{item.snapshot?.paidPrice?.toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="px-2">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm font-medium">
                    ...
                    <span className="text-slate-900 font-bold">₹{order.shippingFee?.toLocaleString()}</span>
                  </div>
                  {order.discount > 0 && (
                  ...
                  <div className="pt-4 mt-2 border-t border-slate-100 flex justify-between items-end">
                    ...
                    <span className="text-2xl font-bold text-indigo-600">₹{order.grandTotal?.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="mt-6 flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-900">{order.payment?.method} • {order.payment?.status}</p>
                  </div>
                </div>
              </div>
...
const Orders = () => {
  ...
  const filteredOrders = orders.filter(order => 
    ...
    order.items?.some(item => item.snapshot?.productName.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    return (
      <div className="min-h-screen bg-[#fcfcfd] pt-24 pb-16 px-4">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {[1, 2, 3].map(i => (
            <div key={i} className="h-32 w-full bg-white border border-slate-100 animate-pulse rounded-2xl"></div>
          ))}
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[11px] font-bold text-indigo-600 uppercase tracking-[0.2em]">Purchase History</span>
            </div>
            <h1 className="text-4xl font-bold text-slate-900 tracking-tight">My Orders</h1>
          </div>
        ...
        {error ? (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-slate-100 shadow-sm px-6">
            ...
            <h3 className="text-xl font-bold text-slate-900">Order Loading Failed</h3>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-slate-100 shadow-sm px-6">
            ...
            {!searchTerm ? (
              <Link to="/" className="mt-10 group flex items-center gap-2 px-10 py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-indigo-600 transition-all active:scale-95 shadow-xl shadow-indigo-500/10">
                Browse Shop
